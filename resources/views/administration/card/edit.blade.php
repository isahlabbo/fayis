  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">EDIT CARD REQUEST </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{route('administration.card.update',[$request->id])}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" name="staffID" value="{{$request->staffID}}" class="form-control" placeholder="Enter your staff ID if applicable"> 
            </div>
            
            <div class="form-group">
                <label for="">Reason for Applying</label>
                <textarea name="reason" id=""  cols="30" rows="5" class="form-control" placeholder="Enter reason for applying ID card">{{$request->reason}}</textarea>
            </div>
