<div class="container-fluid py-3">
    ...
    <div class="card shadow-sm"><div class="card-header bg-white"><strong>Recorded Advance Payments</strong><small class="text-muted d-block">Latest 50 {{strtolower($fee->name)}} advance-payment entries</small></div><div class="table-responsive"><table class="table table-hover mb-0"><thead class="thead-light"><tr><th>Student</th><th>Destination</th><th>Term</th><th>Amount</th><th>Applied</th><th>Credit</th><th>Status</th><th></th></tr></thead><tbody>
        @forelse($recentAdvances as $advance)<tr><td>{{$advance->student->name}}<br><small class="text-muted">{{$advance->student->admission_no}}</small></td><td>{{$advance->academicSession->name}}<br><small>{{optional($advance->sectionClass->section)->name}} / {{$advance->sectionClass->name}}</small></td><td>{{$advance->term->name}}</td><td>{{number_format($advance->amount,2)}}</td><td>{{number_format($advance->applied_amount,2)}}</td><td>{{number_format($advance->remaining_amount,2)}}</td><td><span class="badge badge-{{$advance->status==='Applied'?'success':($advance->status==='Pending'?'warning':'info')}}">{{$advance->status}}</span></td><td><a target="_blank" href="{{route('finance.advance-payments.receipt',$advance->id)}}" class="btn btn-sm btn-outline-primary"><i class="fas fa-print mr-1"></i>Receipt</a> <button type="button" wire:click="deleteAdvancePayment({{$advance->id}})" wire:loading.attr="disabled" onclick="confirm('Delete this advance payment entry? Any amount applied to fees will be reversed. Other entries on this receipt will remain.') || event.stopImmediatePropagation()" class="btn btn-sm btn-outline-danger ml-1"><i class="fas fa-trash mr-1"></i>Delete</button></td></tr>
